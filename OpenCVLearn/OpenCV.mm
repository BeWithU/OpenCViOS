...
//        line(srcMat, rectPoints[i], rectPoints[(i+1)%4], Scalar(0,0,225), 2);
    ...
    for(int i=1;i<rectPoints.size();++i) {
        cv::Point p = rectPoints[i];
        if (p.y < top.y || (p.y == top.y && p.x < top.x)) {
            top = p;
            pointIdx = i;
        }
    }
    rectPoints.erase(rectPoints.begin() + pointIdx);

    //找最下面的点
    pointIdx = 0;
    cv::Point bottom = rectPoints[0];
    for(int i=1;i<rectPoints.size();++i) {
        cv::Point p = rectPoints[i];
    ...
    //两点y轴的关系，是否第一个点是在上的
    if (isTopOfLine[0] && isTopOfLine[1]) {
        //两点都在连线左侧
        srcPoints[0] = rectPoints[0];
        srcPoints[1] = top;
        srcPoints[2] = bottom;
        srcPoints[3] = rectPoints[1];
    } else if (!isTopOfLine[0] && !isTopOfLine[1]) {
        //两点都在连线右侧
        srcPoints[0] = top;
        srcPoints[1] = rectPoints[0];
        srcPoints[2] = rectPoints[1];
        srcPoints[3] = bottom;
    } else {
        //两点分别在左右，让rectPoints[0]在线上面
        //然后根据B值来设置四个顶点
        if (isTopOfLine[1]) {
            swap(rectPoints[0], rectPoints[1]);
        }
